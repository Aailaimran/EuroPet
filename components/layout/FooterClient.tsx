...
import { staggerContainerVariant, cardVariant } from '@/lib/motion'

// ─── Types ────────────────────────────────────────────────────────────────────
export type SiteSettingsLinks = {
  facebookUrl?: string
  instagramUrl?: string
  tiktokUrl?: string
  youtubeUrl?: string
    ...
    return (
      ...
      >
        ...
        <div>
          ...
          <p className="text-gray-400 text-xs mt-0.5">We&apos;ll be in touch with route updates and pet travel news.</p>
        </div>
      </motion.div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
      ...
      <button
        ...
        onMouseEnter={(e) => { if (!isSubmitting) (e.target as HTMLButtonElement).style.backgroundColor = '#b8960c' }}
        onMouseLeave={(e) => { if (!isSubmitting) (e.target as HTMLButtonElement).style.backgroundColor = '#C9A84C' }}
      >
...
// ─── Main Client Footer ───────────────────────────────────────────────────────
export default function FooterClient({ links }: { links: SiteSettingsLinks }) {
  const { ref, isInView } = useScrollAnimation()

  const socialLinks = [
    {
      name: 'Facebook',
      href: links.facebookUrl || 'https://www.facebook.com/share/18HAUnUYuM/?mibextid=wwXIfr',
      color: '#1877F2',
      svg: (
    ...
    {
      name: 'Instagram',
      href: links.instagramUrl || 'https://www.instagram.com/europetexpress26/',
      color: '#E4405F',
      svg: (
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
    ...
    {
      name: 'TikTok',
      href: links.tiktokUrl || 'https://www.tiktok.com/@europet.express',
      color: '#000000',
      svg: (
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
    ...
    {
      name: 'WhatsApp',
      href: `https://wa.me/${links.whatsappNumber || '447853147342'}`,
      color: '#25D366',
      svg: (
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
    ...
    {
      name: 'YouTube',
      href: links.youtubeUrl || 'https://youtube.com/@europetexpress-z1v',
      color: '#FF0000',
      svg: (
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            <div className="text-center lg:text-left max-w-md">
              <p className="text-brand-gold text-xs font-semibold uppercase tracking-[0.2em] mb-2">Stay Connected</p>
              <h3 className="font-playfair text-2xl md:text-3xl font-bold text-white mb-2">Get Route Updates</h3>
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.div variants={cardVariant} className="flex flex-col">
            <Link href="/" className="inline-block" data-cursor="hover">
              <Image src="/UpdatedLogo.png" alt="Euro Pet Express" width={100} height={100} priority={false} className="object-contain h-20 w-auto" />
            </Link>
          ...
          <motion.div variants={cardVariant} className="flex flex-col">
            ...
            <div className="space-y-2 mb-4">
              <a href="tel:+441524959304" className="text-gray-400 hover:text-brand-gold transition-colors text-sm flex items-center gap-2">
        ...
        <div className="border-t border-gray-800 pt-6 mt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-4">
            <Link href="/privacy" className="text-gray-500 text-xs hover:text-brand-gold transition-colors">Privacy Policy</Link>
            <Link href="/terms" className="text-gray-500 text-xs hover:text-brand-gold transition-colors">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
