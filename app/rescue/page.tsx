...
import { RESCUE_DOGS_QUERY, RESCUE_PAGE_QUERY } from '@/sanity/queries'
...
const FALLBACK_DOG_IMAGE = 'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=800&q=80'
...
// Hardcoded fallback dogs shown when Sanity has no dogs seeded
const FALLBACK_DOGS: DogProfile[] = [
  {
    id: 1,
    name: 'Luna',
    ...
    goodWithCats: false,
    needsGarden: true,
    personality: ['Gentle and affectionate', 'Loves outdoor walks', 'Calm indoors', 'Great with adults'],
  ...
  },
  {
    ...
    goodWithCats: false,
    needsGarden: true,
    personality: ['Loyal and protective', 'Loves to play fetch', 'Good with children', 'Energetic and fun'],
    description: 'Bruno is a big-hearted shepherd mix who was surrendered to a Serbian shelter when his owner could no longer care for him. He is fully vaccinated, microchipped, and neutered. Bruno is brilliant with children and other dogs — he just needs a family to call his own.',
...
function DogProfileCard({ dog }: { dog: DogProfile }) {
  return (
...
export default async function RescuePage() {
  const rescuePageCms = await sanityFetch<any>(RESCUE_PAGE_QUERY)
  // Fetch rescue dogs from Sanity CMS
  const sanityDogs = await sanityFetch<any>(RESCUE_DOGS_QUERY)

  // Map Sanity dogs to our DogProfile shape, resolving photo URLs
  const dogs: DogProfile[] = (sanityDogs && sanityDogs.length > 0)
    ? sanityDogs.map((d: {
        _id: string; name: string; breed?: string; gender?: string; age?: string;
        size?: string; location?: string; rescueOrganisation?: string; status: string;
        photo?: object; description?: string; personality?: string[];
        goodWithKids?: boolean; goodWithDogs?: boolean; goodWithCats?: boolean; needsGarden?: boolean;
      }) => ({
        id: d._id,
        name: d.name,
        breed: d.breed || '',
        ...
        size: d.size || '',
        location: d.location || '',
        rescueOrg: d.rescueOrganisation || '',
        status: d.status,
        goodWithKids: d.goodWithKids ?? false,
        goodWithDogs: d.goodWithDogs ?? false,
        goodWithCats: d.goodWithCats ?? false,
        needsGarden: d.needsGarden ?? false,
        personality: d.personality || [],
        description: d.description || '',
        // Use Sanity image if uploaded, otherwise show a friendly fallback
        image: d.photo ? urlForImage(d.photo as Parameters<typeof urlForImage>[0]) || FALLBACK_DOG_IMAGE : FALLBACK_DOG_IMAGE,
        alt: `${d.name} — rescue dog available for adoption`,
      }))
    : FALLBACK_DOGS
  ...
  return (
    <div>
      {/* SECTION 1: PAGE HERO */}
      <PageHero
        title={rescuePageCms?.pageHeading || "Rescue a Dog"}
        subtitle={rescuePageCms?.pageSubheading || "Meet dogs across Europe looking for their forever home. Give a rescued dog the life they deserve."}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
            ...
            <div className="lg:col-span-2">
              ...
              <h2 className="font-playfair text-2xl md:text-3xl font-bold text-white mb-4 leading-tight">
                {rescuePageCms?.missionStatement || "Every dog you see here is waiting for someone like you."}
              </h2>
              <div className="w-16 h-0.5 bg-brand-gold mb-5" />
              <p className="text-gray-400 text-base leading-relaxed">
                {rescuePageCms?.missionBody || "We work with registered rescue organisations across Romania, Serbia, Hungary, Croatia and beyond. These dogs have been abandoned, surrendered, or found on the streets. They have been assessed, vaccinated, microchipped and loved in foster care — and now they are ready for their forever home. When you find a dog you connect with, get in touch. We can help arrange everything — from the adoption process with the rescue organisation to safe transport to your door in the UK."}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
                <p className="font-playfair text-3xl font-bold text-brand-gold mb-1">50+</p>
                <p className="text-gray-400 text-sm">Dogs rehomed</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
                <p className="font-playfair text-2xl md:text-3xl font-bold text-brand-gold mb-1">Multiple</p>
                <p className="text-gray-400 text-sm">Rescue Centres</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
                <p className="font-playfair text-2xl font-bold text-brand-gold mb-2">✓</p>
                <p className="text-gray-400 text-sm">DEFRA Compliant Transport</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-brand-cream py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-14">
            <p className="text-brand-gold text-xs font-semibold uppercase tracking-[0.2em] mb-3">
              {rescuePageCms?.dogsHeading || "Meet The Dogs"}
            </p>
            <h2 className="font-playfair text-3xl md:text-4xl font-bold text-brand-dark mb-4">
              {rescuePageCms?.dogsSubheading || "Looking for their forever home"}
            </h2>
      ...
      <section className="bg-brand-dark py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {[
              { step: '01', title: 'Choose a Dog', desc: 'Browse our rescue dogs and find one that feels right for your home and lifestyle. Not sure? Tell us what you are looking for and we will help match you.' },
              { step: '02', title: 'Submit an Enquiry', desc: 'Click "Request More Information" and tell us about yourself, your home, and which dog you are interested in. Include a message about why you would be a great match.' },
              { step: '03', title: 'Adoption Approval', desc: 'We connect you with the rescue organisation. They will assess your application. Once approved, we arrange all the transport documentation.' },
              { step: '04', title: 'Welcome Home', desc: 'Your dog travels safely with Euro Pet Express. You receive regular photo updates throughout the journey until your new companion arrives at your door.' },
            ].map((item) => (
              <div key={item.step} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-brand-gold/30 hover:bg-white/[0.08] transition-all duration-300">
                <div className="flex items-center gap-3 mb-4">
                  <span className="font-playfair text-3xl font-bold text-brand-gold/30">{item.step}</span>
                </div>
